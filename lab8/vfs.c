...
void init_rootfs() {
  ...
  // file_t *f = memory_pool_allocator(sizeof(file_t), 0);
  // vfs_open("/boot/FAT_R.TXT", O_CREAT, &f);
  // int len1 = 11;
  // char buf1[len1];
  // len1 = vfs_read(f, buf1, len1 - 1);
  // buf1[len1] = 0;
  // uart_sendline("[init_rootfs] Read: %s\n", buf1);
  // int len2 = 1000;
  // char buf2[len2];
  // len2 = vfs_read(f, buf2, len2 - 1);
  // buf2[len2] = 0;
  // uart_sendline("[init_rootfs] Read: %s\n", buf2);
  // vfs_close(f);

  // vfs_mkdir("/boot/TEST");
  // vfs_create("/boot/TEST/FILE1.TXT");
  // vfs_create("/boot/TEST/FILE2.TXT");
  // vfs_create("/boot/TEST/FILE3.TXT");
  // vfs_create("/boot/TEST/FILE4.TXT");
  // vfs_create("/boot/TEST/FILE5.TXT");
  // vfs_create("/boot/TEST/FILE6.TXT");
  // vfs_create("/boot/TEST/FILE7.TXT");
  // vfs_create("/boot/TEST/FILE8.TXT");
  // vfs_create("/boot/TEST/FILE9.TXT");
  // vfs_create("/boot/TEST/FILE10.TXT");
  // vfs_create("/boot/TEST/FILE11.TXT");
  // vfs_create("/boot/TEST/FILE12.TXT");
  // vfs_create("/boot/TEST/FILE13.TXT");
  // vfs_create("/boot/TEST/FILE14.TXT");
  // vfs_mkdir("/boot/TEST/TEST1");
  // vfs_create("/boot/TEST/FILE15.TXT");
  // vfs_create("/boot/TEST/FILE16.TXT");
  // vfs_create("/boot/TEST/FILE17.TXT");
  // vfs_create("/boot/TEST/FILE18.TXT");
  // vfs_create("/boot/TEST/TEST1/FILE19.TXT");
  // vfs_create("/boot/TEST/TEST1/FILE20.TXT");
  // find_filesystem("fat32fs")->syncfs();

  // Only support 1 cluster
  // file_t *f = memory_pool_allocator(sizeof(file_t), 0);
  // vfs_open("/boot/TEST.TXT", O_CREAT, &f);
  // int len = 200;
  // char buf[len];
  // memset(buf, 'a', len);
  // vfs_write(f, buf, len);
...
void parse_directory_structure(vnode_t *node, int level) {
  ...
  if (node->type == TMP) {
    ...
    if (inode->type == DIR) {
      ...
      if (node->mount && node->mount->root) {
        ...
        } else if (node->mount->root->type == INITRAM) {
        ...
        } else if (node->mount->root->type == FAT32) {
          for (int i = 0; i < FAT32FS_MAX_DIR_ENTRY; ++i) {
            vnode_t *child =
                ((fat32_inode_t *)node->mount->root->internal)->entry[i];
            if (child) {
              parse_directory_structure(child, level + 1);
            }
          }
  ...
  } else if (node->type == FAT32) {
    fat32_inode_t *inode = (fat32_inode_t *)node->internal;
    uart_sendline("|-- %s", inode->name);
    if (inode->type == DIR) {
      uart_sendline(" {\n");
      for (int i = 0; i < FAT32FS_MAX_DIR_ENTRY; ++i) {
