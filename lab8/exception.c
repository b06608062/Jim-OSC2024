...
void el0_sync_router(trapframe_t *tpf) {
  uint64_t esr_el1;
  __asm__ __volatile__("mrs %0, esr_el1" : "=r"(esr_el1));
  // esr_el1: Holds syndrome information for an exception taken to EL1.
  // 0x 20 || 0x 24
  esr_el1_t *esr = (esr_el1_t *)&esr_el1;
  if (esr->ec == MEMFAIL_INST_ABORT_LOWER ||
      esr->ec == MEMFAIL_DATA_ABORT_LOWER) {
  ...
  if (syscall_no == 0) {
    ...
    sys_mount(tpf, (char *)tpf->x0, (char *)tpf->x1, (char *)tpf->x2, tpf->x3,
              (void *)tpf->x4);
...
void el1_sync_router(trapframe_t *tpf) {
  uart_sendline("el1_sync_router\n");
  uint64_t esr_el1;
  __asm__ __volatile__("mrs %0, esr_el1\n" : "=r"(esr_el1));
  // esr_el1: Holds syndrome information for an exception taken to EL1.
  esr_el1_t *esr = (esr_el1_t *)&esr_el1;
  // 0x 21 || 0x 25
  if (esr->ec == 0x21 || esr->ec == 0x25) {
    mmu_memfail_abort_handler(esr);
    return;
  }
}
