...
void shell_run() {
  ...
  while (1) {
    ...
    if (strcmp(token, "help") == 0) {
      ...
      break;
    } else if (strcmp(token, "dev_uart") == 0) {
      char *msg = strtok(NULL, " ", &saveptr);
      do_cmd_dev_uart(msg);
    } else {
...
void do_cmd_help() {
  ...
  format_command(" dev_uart <msg>", "Write message to UART device.");
...
void do_cmd_thread() {
  for (int i = 0; i < 5; ++i) {
    thread_t *new_thread = thread_create(thread_test, 0x1000);
...
void do_cmd_dev_uart(const char *msg) {
  file_t *f = memory_pool_allocator(sizeof(file_t), 0);
  vfs_open("/dev/uart", 0, &f);
  char *buf = memory_pool_allocator(strlen(msg) + 3, 0);
  strcpy(buf, msg);
  strcat(buf, "\r\n");
  f->f_ops->write(f, buf, strlen(buf));
  vfs_close(f);
}