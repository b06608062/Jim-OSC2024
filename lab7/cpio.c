...
unsigned long cpio_hexstr_to_ulong(const char *hex, int length) {
...
int cpio_is_directory(const char *mode) {
  unsigned long mode_val = cpio_hexstr_to_ulong(mode, 8);
  return (mode_val & 0170000) == 0040000;
}

int cpio_parse_header(const char *filename) {
  ...
  while (header != NULL) {
    ...
    if (filename == NULL) {
      ...
      if (cpio_is_directory(header->c_mode)) {
        uart_sendline(
            "[CPIO Error]: 'cat' cannot be used to display directory.\n");
      } else {
        uart_sendline("%s\n", filedata);
      }
      return 0;
...
char *cpio_get_file_data(const char *filename) {
  cpio_newc_header_t *header = cpio_header;
  while (header != NULL) {
    if (strncmp(header->c_magic, CPIO_NEWC_MAGIC, sizeof(header->c_magic)) !=
        0) {
  ...
  uart_sendline("[CPIO Error] File not found.\n");
  return NULL;
}

unsigned long cpio_get_file_size(const char *filename) {
  cpio_newc_header_t *header = cpio_header;
  while (header != NULL) {
    if (strncmp(header->c_magic, CPIO_NEWC_MAGIC, sizeof(header->c_magic)) !=
        0) {
