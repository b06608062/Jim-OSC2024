...
thread_t *thread_create(void *entry_point, uint32_t size) {
  ...
  new_thread->context.pgd = (char *)buddy_system_allocator(0x1000);
  simple_memset((void *)(new_thread->context.pgd), 0, 0x1000);
...
void kill_zombies() {
  ...
  double_linked_for_each(cur, run_queue) {
    if (((thread_t *)cur)->state == THREAD_ZOMBIE) {
      ...
      free_page_tables((uint64_t *)thread->context.pgd, 0);
      buddy_system_free((uint64_t)thread->user_space);
    }
  }
  unlock();
}
...
int exec_thread(char *data, uint32_t size) {
  ...
  mappages((size_t *)new_thread->context.pgd, USER_SPACE_BASE,
           new_thread->user_data_size,
           (size_t)VIRT_TO_PHYS(new_thread->user_space), 0);

  mappages((size_t *)new_thread->context.pgd, USER_STACK_BASE - USTACK_SIZE,
           USTACK_SIZE, (size_t)VIRT_TO_PHYS(new_thread->user_stack), 0);

  mappages((size_t *)new_thread->context.pgd, PERIPHERAL_START,
           PERIPHERAL_END - PERIPHERAL_START, PERIPHERAL_START, 0);

  mappages((size_t *)new_thread->context.pgd, USER_SIGNAL_WRAPPER_VA, 0x2000,
  ...
  new_thread->context.lr = USER_SPACE_BASE;
