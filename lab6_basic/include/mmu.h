...
#define TCR_CONFIG_REGION_48bit                                                \
  (((64 - 48) << 0) |                                                          \
   ((64 - 48)                                                                  \
    << 16)) // T0SZ[5:0]   The size offset for ttbr0_el1 is 2**(64-T0SZ):
            // 0x0000_0000_0000_0000 <- 0x0000_FFFF_FFFF_FFFF
#define TCR_CONFIG_4KB                                                         \
  ((0b00 << 14) |                                                              \
   (0b10 << 30)) // T1SZ[21:16] The size offset for ttbr1_el1 is 2**(64-T1SZ):
                 // 0xFFFF_0000_0000_0000 -> 0xFFFF_FFFF_FFFF_FFFF
#define TCR_CONFIG_DEFAULT                                                     \
  (TCR_CONFIG_REGION_48bit |                                                   \
   TCR_CONFIG_4KB) // TG0[15:14]  Granule size for the TTBR0_EL1: 0b00 = 4KB //
                   // TG1[31:30]  Granule size for the TTBR1_EL1: 0b10 = 4KB

#define MAIR_DEVICE_nGnRnE                                                     \
  0b00000000 // ((MAIR_DEVICE_nGnRnE << (MAIR_IDX_DEVICE_nGnRnE * 8)) |
             // (MAIR_NORMAL_NOCACHE << (MAIR_IDX_NORMAL_NOCACHE * 8)))
#define MAIR_NORMAL_NOCACHE                                                    \
  0b01000100 // mair_el1: Provides the memory attribute encodings corresponding
             // to the possible AttrIndx values for stage 1 translations at EL1.
#define MAIR_IDX_DEVICE_nGnRnE                                                 \
  0 // ATTR0[7:0]: 0b0000dd00 Device memory,   dd = 0b00   Device-nGnRnE memory
#define MAIR_IDX_NORMAL_NOCACHE                                                \
  1 // ATTR1[14:8] 0booooiiii Normal memory, oooo = 0b0100 Outer Non-cacheable,
...
#define USER_SPACE_BASE 0x00000000L
...
#define BOOT_PTE_ATTR_nGnRnE                                                   \
  (PD_BLOCK | PD_ACCESS | (MAIR_DEVICE_nGnRnE << 2) | PD_UNX | PD_KNX |        \
   PD_UK_ACCESS) // p.17
#define BOOT_PTE_ATTR_NOCACHE                                                  \
  (PD_BLOCK | PD_ACCESS | (MAIR_NORMAL_NOCACHE << 2))
