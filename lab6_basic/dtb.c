...
void dtb_traverse_device_tree() {
  ...
  if (dtb_be32_to_le32(hdr->magic) != FDT_MAGIC) {
    uart_sendline("[Device Tree Error] Invalid device tree blob header.\n");
    return;
  }
  ...
  while (p < struct_end) {
    ...
    case FDT_END:
      ...
      return;
    default:
      uart_sendline("[Device Tree Error] Unknown token.\n");
      return;
    }
  }
}
...
void dtb_initramfs_init() {
  ...
  while (p < struct_end) {
    ...
    if (token == FDT_BEGIN_NODE) {
      ...
      if (strcmp(name, "linux,initrd-start") == 0) {
        cpio_header = (cpio_newc_header_t *)(unsigned long)PHYS_TO_VIRT(
            dtb_be32_to_le32(*(uint32_t *)(prop_data + 1)));
        ...
        cpio_end = (char *)(unsigned long)PHYS_TO_VIRT(
            dtb_be32_to_le32(*(uint32_t *)(prop_data + 1)));
      }
      p += sizeof(fdt_prop_data_t) + ((len + 3) & ~3);
  ...
  uart_sendline("[Device Tree Error] Initramfs start address not found.\n");
}
...
void dtb_reserve_memory() {
  ...
  while (dtb_be64_to_le64(rsv_entry->address) != 0 ||
    ...
    uint64_t start = PHYS_TO_VIRT(dtb_be64_to_le64(rsv_entry->address));
